...
# GJ436 is 33.268 light years away and has 0.464 solar radius
# GJ1214 is 47.8 light years away and has 0.2064 solar radius
...
hdulist = fits.open('hlsp_muscles_multi_multi_gj436_broadband_v22_adapt-const-res-sed.fits')
print (hdulist.info())
spec = fits.getdata('hlsp_muscles_multi_multi_gj436_broadband_v22_adapt-const-res-sed.fits', 1)
...
for n,wl in enumerate(spec['WAVELENGTH']):
    new_str += '{:<14.10f}'.format(wl*0.1) + "{:>14.4E}".format(float(spec['FLUX'][n]*10. *( (33.268*(63241.1*au))/(r_sun*0.464) )**2 )) + '\n'
    #new_str += '{:<12}'.format(wl*0.1) + "{:>12.2E}".format(float(spec['FLUX'][n]*10. *(31.8*(63241.1*au)/ (r_sun*0.42) )**2        )) + '\n'
...
with open('sflux-GJ436.txt', 'w+') as f: f.write(new_str)   
