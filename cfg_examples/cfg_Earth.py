...
# input:
network = 'thermo/SNCHOAr_full_photo_network_2025.txt'
...
output_dir = 'output/'
plot_dir = 'plot/'
movie_dir = 'plot/movie/Earth-post025/'
out_name =  'Earth-2N7bar-nz120-post075.vul' # output file name
...
Rp = 6.3781e8 # Planetary radius (cm) (for computing gravity) | R_jup = 7.1492E9 cm by IAU 2015
Mp = 5.9722e27  # Planetary mass (cm) (for computing gravity)   | M_jup = 1.8981E30 g by IAU 2015
...
P_t = 2e-1 # pressure at the top (dyne/cm^2)
use_Kzz = True
use_vz = False
...
use_topflux = False
use_botflux = True
use_fix_sp_bot = {"H2O":0.00894, "H2O_l_s":0, 'CO2':4E-4} # fixed mixing ratios at the lower boundary
...
use_settling = True
start_conden_time = 0
stop_conden_time = 3e6 #3e8 # 10 yrs   # after this time to fix the condensable species
condense_sp = ["H2O" , "H2SO4", "S8"]    
non_gas_sp = [ 'H2O_l_s', "H2SO4_l" , "S8_l_s"]
r_p = {'H2O_l_s': 5e-3, 'H2SO4_l':5e-5, "S8_l_s":1e-5}  # particle radius in cm (1e-4 = 1 micron)
rho_p = {'H2O_l_s': 1, 'H2SO4_l':1.8302, "S8_l_s":2.07} # particle density in g cm^-3
fix_species = ["H2O" , "H2SO4", "S8", 'H2O_l_s', "H2SO4_l" , "S8_l_s"]      # fixed the condensable species after condensation-evapoation EQ has reached  
...
st_factor = 0.5
conv_step = 500
conver_ignore = ['HC3N', 'H'] # added 2023. to get rid off non-convergent species, e.g. HC3N without sinks ### use with caution ###

# ====== Setting up numerical parameters for Ros2 ODE solver ====== 
use_adapt_rtol = True
rtol = 1.5            # relative tolerence for adjusting the stepsize 
post_conden_rtol = 0.075 # switched to this value after fix_species_time
rtol_min = 0.01
rtol_max = 2.5
...
plot_spec = ['H2O', 'O3', 'CH4', 'CO', 'CO2', 'SO2' ,'H2SO4','H']
# output:
